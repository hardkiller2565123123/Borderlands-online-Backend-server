...
namespace bolemu
{
    bool SendPhotonPresenceResponse(
        ...
        bool encrypted)
    {
        // Assembly-CSharp.dll -> PresenceService::.ctor registers operation
        // 147 (0x93) with PresenceService::OnGetPresenceResponse. That callback
        // reads response parameter 0 and casts it to System.Collections.Hashtable,
        // then constructs a Presence from it. An empty Hashtable is a safe local
        // baseline: the managed Presence object keeps its normal default values
        // and, most importantly, the async request is allowed to complete.
        ...
        std::printf("[PHOTON/UDP] GetPresence success: local/default Presence for character id=%u ch=0 seq=%u%s\n",
