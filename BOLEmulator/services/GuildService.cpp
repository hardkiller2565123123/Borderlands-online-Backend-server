...
namespace bolemu
{
    bool SendPhotonMyGuildIdResponse(
        SOCKET server,
        ...
        bool encrypted)
    {
        // Assembly-CSharp.dll -> OperationCode.GetMyGuildId = 102 (0x66).
        // GuildService::OnMyGuildIdCallback reads response parameter 0 as an int.
        // The game's own GuildServiceOffline path reports -1 when the character
        // has no guild, so preserve that native no-guild convention locally.
        ...
        plain.push_back(0);    // parameter key 0 = guild id
        AppendProtocol16TypedInt(plain, -1); // no guild
        ...
        if (encrypted)
        {
            ...
            {
                std::printf("[PHOTON/UDP] GET_MY_GUILD_ID response encryption failed\n");
                std::fflush(stdout);
                return false;
            }
        ...
        {
            std::printf("[PHOTON/UDP] GET_MY_GUILD_ID response send failed: %d\n", WSAGetLastError());
            std::fflush(stdout);
            return false;
        }

        PrintHex("[PHOTON/UDP] guild-id response plaintext", plain.data(), static_cast<int>(plain.size()));
        PrintHex("[PHOTON/UDP] -> GET_MY_GUILD_ID_RESPONSE", reply.data(), static_cast<int>(reply.size()));
        std::printf("[PHOTON/UDP] GetMyGuildId success: character id=%u has no guild (guildId=-1) ch=0 seq=%u%s\n",
                    g_localCharacter.id,
                    sequence,
                    encrypted ? " encrypted" : "");
        std::printf("[PHOTON/UDP] waiting for the next post-guild-id BOL operation\n");
        std::fflush(stdout);
        return true;
    }
}
